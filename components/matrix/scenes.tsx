'use client';
// NEON GRID v3 set pieces — a drivable city. Streets have lane lights and
// signposts; districts hold the résumé. Glow = toneMapped=false past bloom.
import { useEffect, useMemo, useRef } from 'react';
import { useFrame } from '@react-three/fiber';
import * as THREE from 'three';
import {
  GX, NEONS, STREETS, JUNCTIONS, GATE_ARCH, IDENTITY_SPOT, ARSENAL_SPOT,
...
// ── canvas texture helpers (no green anywhere) ──────────────────────────────
...
// ── city blocks — buildings lining every street ─────────────────────────────
export function CityBlocks({ tier }: { tier: 'S' | 'M' | 'L' }) {
  const step = tier === 'S' ? 22 : tier === 'M' ? 15 : 11;
  ...
  const { body, trim } = useMemo(() => {
    ...
    STREETS.forEach((s, si) => {
      ...
      const ux = dx / len, uz = dz / len;       // along street
      const nx = -uz, nz = ux;                  // perpendicular
      for (let d = 8; d < len - 4; d += step) {
        for (const side of [-1, 1]) {
          const seed = si * 997 + d * 13 + side * 7;
          const off = 15 + rnd(seed + 1) * 12;
          slots.push({ x: ax + ux * d + nx * side * off, z: az + uz * d + nz * side * off, seed });
        }
      }
    });
...
export function StreetLanes() {
  ...
  return (
    <group>
      ...
      <gridHelper args={[700, 140, '#12224A', '#0A0F24']} position={[0, 0.01, -200]} />
      {lanes.map((l, i) => (
...
// ── city gates — neon arch over the avenue (the hero backdrop) ──────────────
...
// ── junction signposts — directional boards to every district ───────────────
export function Signposts() {
  const posts = useMemo(() => JUNCTIONS.map((j, i) => {
    const name = j.left ?? j.right ?? '';
    const arrow = j.left ? '◀' : '▶';
    const side = j.left ? -1 : 1;
    const color = NEONS[i % NEONS.length];
    return {
      x: side * 5.6, z: j.z + 6.5,
      tex: textTexture([{ text: `${arrow} ${name}`, size: 58, color }], 760, 120, color),
    };
  }), []);
  return (
    <group>
      {posts.map((p, i) => (
        <group key={i} position={[p.x, 0, p.z]}>
          ...
          <mesh position={[0, 4.6, 0]}>
            <planeGeometry args={[6.4, 1.05]} />
            <meshBasicMaterial map={p.tex} transparent side={THREE.DoubleSide} />
          </mesh>
        </group>
      ))}
    </group>
...
// ── identity plaza — floating holo ID + light beam ──────────────────────────
...
// ── arsenal wall — flickering neon signs at the end of its street ───────────
...
// ── era portals — rings of light you punch through (no spirals) ─────────────
...
export function EraPortals() {
  return <>{PORTAL_XS.map((_, i) => <Portal key={i} index={i} />)}</>;
}

// ── warp speed-lines (only while crossing a portal) ─────────────────────────
...
function Billboard({ index, stabilized, onStabilize }: { index: number; stabilized: boolean; onStabilize: () => void }) {
  ...
  return (
    <group ref={group} position={[spot.x, 6, spot.z]} rotation={[0, index === 0 ? 0.4 : -0.5, 0]}>
      <mesh
...
// ── credentials court — floating verified plaques ───────────────────────────
export function CredsCourt({ reduced }: { reduced: boolean }) {
  const cards = useMemo(() => {
    const list = [
      ...certifications.map((c, i) => ({
        tex: textTexture([
          { text: `[${c.badge}] VERIFIED ✓`, size: 44, color: GX.blueBright },
          { text: c.title.toUpperCase(), size: 40, color: GX.white },
          { text: c.issuer.toUpperCase(), size: 30, color: GX.violetBright },
        ], 900, 420, GX.blue),
        dx: i === 0 ? -4.5 : 4.5, dz: i === 0 ? -3 : -1, y: 5.4,
      })),
      {
        tex: textTexture([
  ...
  useFrame((state) => {
    ...
    refs.current.forEach((g, i) => {
      ...
      g.rotation.y = Math.PI / 2 + Math.sin(t * 0.35 + i) * 0.1;
    });
  });
  return (
    <group>
      {cards.map((c, i) => (
        <group key={i} ref={(g) => { refs.current[i] = g; }} position={[CREDS_SPOT.x + c.dx, c.y, CREDS_SPOT.z + c.dz]} rotation={[0, Math.PI / 2, 0]}>
          <mesh>
...
// ── transmission row — neon boards for the blog ─────────────────────────────
export function TransmissionRow() {
  ...
  return (
    <group>
      {boards.map((b, i) => (
        <group key={i} position={[TRANS_SPOT.x, 5 + (i % 2) * 2.4, TRANS_SPOT.z + b.dz]} rotation={[0, Math.PI / 2, 0]}>
          <mesh>
...
// ── the choice — end-of-avenue pills ────────────────────────────────────────
...
// ── the white rabbit — leads you down the streets ───────────────────────────
